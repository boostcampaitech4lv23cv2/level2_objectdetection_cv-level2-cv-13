...
def parse_args():                                                           # set a argument parser
    ...
    parser.add_argument(
        ...
        default='/opt/ml/level2_objectdetection_cv-level2-cv-13/fold_dataset/train_fold_4_of_5.json')
    parser.add_argument(
        '--csv-dir',
        type=str,
        help='The csv file directory of model_output.csv',
        default='/opt/ml/level2_objectdetection_cv-level2-cv-13/ensemble_result/ensemble.csv')
    parser.add_argument(
        '--out-dir',
        type=str,
        help='The directory of output json file.',
        default='/opt/ml/level2_objectdetection_cv-level2-cv-13/pseudo_labeling/swinb_f4_0.975.json')
    parser.add_argument(
        '--threshold',
        type=float,
        default=0.975
...
# pseudo labeling
def pseudo_labeling(empty_json_dir, csv_dir, out_dir, threshold):
    ...
    with open(empty_json_dir, encoding='utf-8', errors='ignore') as json_data:    # json file open
        ...
        for i in tqdm(range(len(df))):                                           # append new json data for each image
            ...
            data["images"].append({                                         # append a new images row
                    ...
                    "coco_url": None,
                    "date_captured": None,
                    "id": int(df['image_id'][i].split('/')[1].split('.')[0])+5000
                })
            ...
            for j in range(ann_num):                                        # append new json data for each object annotations
                if float(ann[j*6 + 1]) >= threshold:
                    ...
                    data['annotations'].append({                                # append a new annotations row
                        "image_id": int(df['image_id'][i].split('/')[1].split('.')[0])+5000,
...
if __name__ =="__main__":
    args = parse_args()
    pseudo_labeling(args.empty_json_dir, args.csv_dir, args.out_dir, args.threshold)